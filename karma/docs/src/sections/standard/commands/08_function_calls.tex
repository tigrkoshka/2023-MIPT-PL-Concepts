...
\cmdtable{Function call comands}{
    ...
    49 & \St{call} & \Ss{RR} & \RRcmd{call}{r0}{r5}{2} &
    Call the function located \newline
    at the address \St{$\text{r5} + \text{2}$} \\

    \hline

    50 & \St{calli} & \Ss{J} & \St{calli\kern 0.5em 21913} &
    Call the function located \newline
    at the address \St{21913} \\

    \hline

    51 & \St{ret} & \Ss{J} & \St{ret\kern 0.35em 3} &
...
\begin{itemize}
    ...
    \item of the instruction that would be executed next if the current command
    \item did not interrupt the consecutive execution, the \textit{return point})
    \item to the stack (as if with the \hyperlink{cmd:push}{\St{push}} command)
